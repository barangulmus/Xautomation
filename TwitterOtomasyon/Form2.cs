...
namespace TwitterOtomasyon
{
	public partial class Form2 : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			{
				DialogResult msg = MessageBox.Show("This process may take some time. Do you want to continue? Also, please refrain from using the mouse and keyboard while the program is running for it to function properly.", "Uyarı", MessageBoxButtons.YesNo);
					...
					if (r3)
					{
						...
						if (int.TryParse(numericPart, out int postCount))
						{
							for (int i = 0; i < postCount; i++)
							{
								...
								{
									MessageBox.Show("An Eerror Occurred" + ex);
									throw;
								}
								finally
								{
						...
						{
							MessageBox.Show("Failed to Convert to a Numbe");
						}
					}
					if (r4)
					{
			...
			{
				MessageBox.Show("Please make sure you haven't left any blank spaces.");
			}
		}
		...
		private void button4_Click(object sender, EventArgs e)
		{
			if (listBox1.Items.Count > 0)
			{
				...
				MessageBox.Show("Items have been copied to the bathroom!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
			else
			{
				MessageBox.Show("No item to copy!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
		}
        ...
        {
            panel3.BackColor = Color.FromArgb(10, 0, 0, 0);
            panel4.BackColor = Color.FromArgb(10, 0, 0, 0);
            panel5.BackColor = Color.FromArgb(10, 0, 0, 0);
            panel6.BackColor = Color.FromArgb(10, 0, 0, 0);
            groupBox1.BackColor = Color.FromArgb(10, 0, 0, 0);
            groupBox2.BackColor = Color.FromArgb(10, 0, 0, 0);
            groupBox3.BackColor = Color.FromArgb(10, 0, 0, 0);
            groupBox5.BackColor = Color.FromArgb(10, 0, 0, 0);
            groupBox6.BackColor = Color.FromArgb(10, 0, 0, 0);
        }
				...
				if (int.TryParse(followNumericPart, out int followCount))
				{
					...
					for (int i = 1; i <= followCount - 1; i++)
					{
						try
						{
							...
							if (r9)
							{
								By thefollowBYYX = By.XPath($"((//div[contains(@style, 'position: relative;')]//div[@data-testid='cellInnerDiv' and descendant::div/div])//div[@data-testid='UserCell']/div/div[2]/div/div/div/div[2]//a[@href='/{PersonText}'])/parent::div/following-sibling::div");
								if (driver.FindElements(thefollowBYYX).Count <= 0)
								{
									System.Threading.Thread.Sleep(500);
									IWebElement theFollow = driver.FindElement(By.XPath($"(//div[contains(@style, 'position: relative;')]//div[@data-testid='cellInnerDiv' and descendant::div/div])//div[@data-testid='UserCell']/div/div[2]/div/div/div/div[2]//a/parent::div/parent::div/parent::div/parent::div/following::div/div[@role='button' and contains(@aria-label, '{PersonText}')]"));
									theFollow.Click();
						...
						{
							listBox2.DataSource = followList;
							MessageBox.Show("An Error Occurred, Found Account Count" + i + " " + PersonText + "\n" + ex);
							throw;
						}
						finally
						{
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			By tweetClickLocator = By.XPath("//a[@href='/compose/tweet']");
			for (int i = 1; i <= how_many_twits; i++)
			{
				IWebElement clicktosend = driver.FindElement(tweetClickLocator);
				clicktosend.Click();
				System.Threading.Thread.Sleep(500);
				SendKeys.Send(tweetText + " " + i);
				System.Threading.Thread.Sleep(500);
				IWebElement tweetClick = driver.FindElement(By.XPath("//div[@role='button' and @data-testid='tweetButton']"));
				tweetClick.Click();
				System.Threading.Thread.Sleep(3000);
			}
		}
